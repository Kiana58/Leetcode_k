class Solution(object):
    def findLadders(self, beginWord, endWord, wordList):
        ...
        # records of shortest path from biginWord for each word in wordList
        distance = {}
    ...
    def bfs(self, start, fromToDict, distance, d):
        ...
        while queue:
            ...
            # for word in the wordList which are just one char diff with curr_word
            for next_word in self.get_next_words(curr_word, d):
                # if next_word not shown before or next_word distance like current path, there is a path from curr_word to next_word
                if (next_word not in distance or distance[next_word] == distance[curr_word]+1):
                    fromToDict[curr_word].append(next_word)
                    
                # if next_word not shown before, record from start, to every word, the shortest path, then find the word from nextWord
    ...
    # find all words in the wordList/d which are just one char diff with word
